...
export default class LegendList extends Component {
  ...
  componentDidMount = () => {
    ...
    this.props.layers.forEach(layer => {
      statisticsApi.fetch(layer.geospace.id, getToken(), (status, stat) => {
  ...
  matchGeometry = (geometry, type) => {
    ...
    if (typeof geometry !== 'undefined') {
      match = _.some(geometry, el => {
        return el.toLowerCase().indexOf(type.toLowerCase()) !== -1;
      });
    }
  ...
  renderLegendList = ({ id, index, type, label, legend }) => {
    ...
    const borderWidth =
      ...
      legend && legend[0] && legend[0].strokeWidth
        ? 2
        : legend && legend[1] && legend[0].width !== legend[1].width
        ? 2
        : shape === 'Polygon'
        ? 1
        : 0;
    ...
    return (
      <div className={style.wrapper}>
        <div className={style.header}>
          <h2 className={`desktop ${style.title}`}>{this.props.label}</h2>
        </div>
