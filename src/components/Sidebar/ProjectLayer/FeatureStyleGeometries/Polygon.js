...
class Polygon extends Component {
    ...
    return (
      <Draggable key={ruleId} draggableId={ruleId} index={k - 1}>
        {(provided, snapshot) => (
          ...
          >
            {this.getAccordion(ruleId, k, name, provided, 'popup')}
          </div>
        )}
      </Draggable>
    );
  ...
  getAccordion = (ruleId, k, name, provided, action, noshape = true) => {
    ...
    if (
      ...
      this.props.currentLayer.geometryStyle.includes('Polygon')
    )
      active = true;
    return (
      <Accordion
      ...
      >
        <div className={style.wrapper}>
          ...
          {
            //! !this.props.featureProps.length && <TextFeatureStyle
            <TextFeatureStyle
              currentValue={getStyleValue(
                this.props.styleGroups,
                geometryType,
  ...
  getDefaultStyleRulesAccordion = () => {
    const defaultStyleLabel =
      (this.props.currentLayer.geometriesLabels &&
        this.props.currentLayer.geometriesLabels.Polygon &&
        this.props.currentLayer.geometriesLabels.Polygon.defaultStyleLabel) ||
